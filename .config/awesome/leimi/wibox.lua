-- copy of original wibox, just one line@137 commented for my "floating" wibox
-- doesn't update struts when height changes so that we dont have ugly unnecessary redraw on show/hide of floating wibox
-- used for my statusbar
...
function leimiwibox.get_position(wibox)
...
function leimiwibox.set_position(wibox, position, screen)
...
local function update_all_wiboxes_position()
    for _, wprop in ipairs(wiboxes) do
        leimiwibox.set_position(wprop.wibox, wprop.position, wprop.screen)
    end
end
...
function leimiwibox.attach(wibox, position, screen)
...
function leimiwibox.align(wibox, align, screen)
    local position = leimiwibox.get_position(wibox)
...
function leimiwibox.stretch(wibox, screen)
    if screen then
        local position = leimiwibox.get_position(wibox)
        local area = capi.screen[screen].workarea
...
function leimiwibox.new(arg)
    ...
    leimiwibox.attach(w, position, screen)
    if has_to_stretch then
        leimiwibox.stretch(w, screen)
    else
        leimiwibox.align(w, arg.align, screen)
    end

    leimiwibox.set_position(w, position, screen)
...
function leimiwibox.mt:__call(...)
    return leimiwibox.new(...)
end

return setmetatable(leimiwibox, leimiwibox.mt)
