-- copy of /usr/share/awesome/lib/wibox/widget/calendar.lua
-- in order to create a 3 month calendar
...
-- @ADDED by manu
local function create_months(props, date)
    -- Create a grid widget with current month, last month and next month
    local in_layout = grid()
    in_layout:set_expand(true)
    in_layout:set_homogeneous(true)
    in_layout:set_spacing(2*props.spacing)
    in_layout:set_forced_num_cols(3)
    in_layout:set_forced_num_rows(1)

    local month_date
    local current_date = os.date("*t")

    local prev_month_date = current_date.month == 1 and {month = 12, year = current_date.year - 1} or {month = current_date.month - 1, year = current_date.year}
    local month_date = {day=current_date.day, month=current_date.month, year=current_date.year}
    local next_month_date = current_date.month == 12 and {month = 1, year = current_date.year + 1} or {month = current_date.month + 1, year = current_date.year}
    in_layout:add(create_month(props, prev_month_date))
    in_layout:add(create_month(props, month_date))
    in_layout:add(create_month(props, next_month_date))
...
local function fill_container(self)
    ...
    if date then
        ...
        -- @ADDED by manu
        elseif self._private.type == "months" then
            self._private.container:set_widget(create_months(self._private, date))
        end
    else
...
-- @ADDED by manu
function calendar.months(date, font)
    return get_calendar("months", date, font)
end
